...
impl LightInstruction {
    fn from_str(s: &str) -> Self {
        ...
        if split.get(0).unwrap() == &"toggle" {
            ...
            let x1 = c1.split(",").collect::<Vec<&str>>().get(0).unwrap().parse::<usize>().unwrap();
            let y1 = c1.split(",").collect::<Vec<&str>>().get(1).unwrap().parse::<usize>().unwrap();

            let c2 = split.get(3).unwrap();
            let x2 = c2.split(",").collect::<Vec<&str>>().get(0).unwrap().parse::<usize>().unwrap();
            let y2 = c2.split(",").collect::<Vec<&str>>().get(1).unwrap().parse::<usize>().unwrap();
...
fn brighten(lights: &mut [[u32; 1000]; 1000], x1: usize, y1:usize, x2: usize, y2: usize, amount: u32) {
    for x in x1..x2+1 {
        for y in y1..y2+1 {
            lights[x][y] += amount;
        }
